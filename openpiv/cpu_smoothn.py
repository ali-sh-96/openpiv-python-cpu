"""Module for smoothing algorithms.

Output is yet to be compared to original MATLAB version to confirm that the smoothing performance
has been preserved.
...
ALLOWED_WEIGHT_METHODS = {"bisquare", "talworth", "cauchy"}
...
class SmoothnCPU:
    ...
    smoothn automatically smooths the uniformly-sampled array y. y can be any n-D noisy array
    ...
    s : float or None, optional
        ...
        smoother the output will be. If the smoothing parameter s is empty (i.e. s = None), it is automatically
        determined using the generalized cross-validation (GCV) method.
    smoothing_order : {0, 1, 2}, optional
        Order criterion.
    max_steps : int, optional
        Maximum number of robust steps.
    ...
    def __init__(self, y_shape,
        ...
        self.y_size = prod(self.y_shape)
        self.y_ndim = len(self.y_shape)
        self.mask = mask
        self.w = w
        self.w_tot = self.w
        self.z0 = z0
    ...
    def __call__(self, *y):
        ...
        -------
        z : ndarray or list
            ndarray or a list of ndarrays of smoothed values.
        ...
        self.y = list(y) if isinstance(y, tuple) else y
        self.n_y = len(self.y)
        self.y0 = self.y[0]
        
        # Replace missing y-data.
        ...
        lambda_ = self.get_lambda(self.y0, self.spacing) ** self.smoothing_order
        ...
        for robust_step in range(self.max_steps):
            ...
            for iter_n in range(self.max_iters):
                z0 = self.z
                y_dct = [self.get_dct(self.w_tot * (self.y[i] - self.z[i]) + self.z[i], f=dct) for i in range(self.n_y)]
                ...
                if self.is_auto and not np.remainder(log2(iter_n + 1), 1):
                    ...
                    if not p:
                        ...
                        for i, p_i in enumerate(p_span):
                            g[i] = self.get_gcv_score(p_i, self.y, y_dct, self.w_tot, lambda_, w_mean)
                        
                        p = p_span[np.argmin(g)]

                    # Estimate the smoothing parameter.
                    p, _, _ = fmin_l_bfgs_b(self.get_gcv_score, np.array([p]), fprime=None,
                                            factr=10, approx_grad=True, bounds=[(p_min, p_max)],
                                            args=(self.y, y_dct, self.w_tot, lambda_, w_mean))
                    p = p[0]
                
                # Update z using the gamma coefficients.
                s = 10 ** p
                gamma = 1 / (1 + s * lambda_)
                self.z = [relaxation_factor * self.get_dct(gamma * y_dct[i], f=idct) + 
                          (1 - relaxation_factor) * self.z[i] for i in range(self.n_y)]
                
                # If not weighted/missing data, tol=0 (no iteration).
                if self.is_weighted:
                    tol = np.linalg.norm([z0[i] - self.z[i] for i in range(self.n_y)]) / (np.linalg.norm(self.z) + 1e-6)
                else:
                    tol = 0
            ...
            if self.is_robust:
                self.w_tot = self.w * self.get_robust_weights(self.y, self.z, s)
        ...
        if iter_n == self.max_iters - 1:
            max_iters_warning(self.max_iters)
        
        if self.mask is not None:
            self.z = [np.where(self.mask, 0, self.z[i]) for i in range(self.n_y)]
        if self.n_y == 1:
            self.z = self.z[0]
        if self.s is None:
            self.s = s
    ...
    def replace_non_finite(self):
        ...
        # Find missing y-data.
        self.is_finite = np.isfinite(self.y0)
        for i in range(1, self.n_y):
            self.is_finite = self.is_finite * np.isfinite(self.y[i])
        
        self.num_finite = self.is_finite.sum()
        ...
        for i in range(self.n_y):
            if np.any(self.missing):
                ...
                self.y[i][self.missing] = self.y[i][neighbour_index][self.missing]
    ...
    def init_weights(self):
        ...
        if self.w is not None:
            ...
            self.w = np.ones(self.y_shape, dtype=self.dtype_f) * self.is_finite
    ...
    def init_z(self):
        ...
        if self.is_weighted:
            # With weighted/missing data, an initial guess is provided to ensure faster convergence.
            # For that purpose, a nearest neighbor interpolation followed by a coarse smoothing are performed.
            if self.z0 is not None:
                if not isinstance(self.z0, list):
                    self.z0 = [self.z0]
                self.z = self.z0
            else:
                self.z = self.get_init_z(self.y)
        else:
            self.z = [np.zeros(self.y_shape, dtype=self.dtype_f)] * self.n_y
    
    def get_init_z(self, y):
        """Returns initial guess for z using coarse, fast smoothing."""
        # Forward transform.
        z_dct = [self.get_dct(y[i], f=dct) for i in range(self.n_y)]
        ...
        # Backwards transform.
        z = [self.get_dct(z_dct[i], f=idct) for i in range(self.n_y)]
        return z
    
    def get_dct(self, data, f=dct):
        """Returns the nD dct."""
        if self.y_ndim == 1:
    ...
    def init_spacing(self):
        ...
        if self.spacing is not None:
            ...
            self.spacing = np.ones(self.y_ndim)
    ...
    def get_p_bounds(self, smoothing_order=2):
        ...
        rank_y = np.sum(np.array(self.y_shape) != 1)
    ...
    def get_lambda(self, y, spacing):
        """Returns the lambda tensor. lambda contains the eigenvalues of the difference matrix used in this
        penalized-least-squares process."""
        lambda_ = np.zeros(self.y_shape, dtype=self.dtype_f)
        for k in range(self.y_ndim):
            shape_k = np.ones(self.y_ndim, dtype=int)
            shape_k[k] = self.y_shape[k]
            lambda_ += (np.cos(np.pi * (np.arange(1, self.y_shape[k] + 1) - 1) / self.y_shape[k]) / spacing[k] ** 2).reshape(shape_k)
        lambda_ = 2 * (self.y_ndim - lambda_)
        
        return lambda_
    
    def get_gcv_score(self, p, y, y_dct, w, lambda_, w_mean):
        ...
        if w_mean > 0.9:
            ...
            for i in range(self.n_y):
                y_hat = self.get_dct(gamma * y_dct[i], f=idct)
                residual_sum_squares += np.linalg.norm(np.sqrt(w[self.is_finite]) * (y[i][self.is_finite] - y_hat[self.is_finite])) ** 2
        
        # Divide by n_y to match score of scalar field.
        tr_h = np.sum(gamma)
        gcv_score = residual_sum_squares / self.num_finite / (1 - tr_h / self.y_size) ** 2 / self.n_y
        return float(gcv_score)
    
    def get_robust_weights(self, y, z, s):
        ...
        if self.weight_method == "cauchy":
            c = 2.385
            w = 1 / (1 + (u / c) ** 2)
        elif self.weight_method == "talworth":
            c = 2.795
            w = u < c
        else:
            c = 4.685
    ...
    def get_rms(self, x):
        return np.sqrt(np.sum(x ** 2, axis=0) / self.n_y)
