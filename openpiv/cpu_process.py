"""This module contains algorithms for PIV image analysis with CPU."""
...
from .cpu_misc import DTYPE_f, DTYPE_c, get_stack
...
class piv_cpu:
    """Wrapper-class for PIVCPU that further applies input validation.
    ...
    min_search_size : int
        Length of the sides of the square search window. Only supports multiples of 8 and power of 2.
    **kwargs
    ...
    ----------
    coords : tuple
        A tuple of 2D arrays, (x, y) coordinates, where the velocity field has been computed.
    field_mask : ndarray
    ...
    def __init__(self, frame_shape, min_search_size, **kwargs):
        ...
        dtype_f = kwargs["dtype_f"] if "dtype_f" in kwargs else DTYPE_f
            ...
            all(frame.shape == self.frame_shape for frame in frames) and \
                all(np.issubdtype(frame.dtype, np.number) for frame in frames) and \
                    all(not np.iscomplex(frame).any() for frame in frames), \
                        "Both frames must be an ndarray of {} with shape {}.".format("real", self.frame_shape)
...
class PIVCPU:
    ...
    The process involves iteratively utilizing the estimated displacement and gradient to shift and deform the
    interrogation windows for the subsequent iterations. Multiple iterations can be conducted before the estimated
    velocity is interpolated onto a finer mesh. This procedure continues until the desired final mesh and the specified
    ...
    The new displacement is computed by dx = dxp + dcx and dy = dyp + dcy.
    Validation is done by any combination of signal-to-noise ratio, median, median-absolute-deviation (MAD),
    ...
    search_size_iters : int or tuple, optional
        The length of this tuple represents the number of different window sizes to be used, and each entry
        specifies the number of times a particular window size is used.
    overlap_ratio : float or tuple, optional
        Ratio of overlap between two windows (between 0 and 1) for different window sizes.
    ...
    subpixel_method : {"gaussian", "centroid", "parabolic"} or tuple, optional
        Method to estimate subpixel location of the peak at each iteration.
    n_fft : int or tuple, optional
        Size-factor of the 2D FFT. n_fft of 2 is recommended for the last iteration.
    deforming_par : float or tuple, optional
        ...
        A default value of 0.5 is recommended to reduce bias errors. A value of 1 corresponds to only
    ...
    s2n_tol : float or None or tuple, optional
        Tolerance for signal-to-noise (S2N) validation at every iteration.
    median_tol : float or None or tuple, optional
        Tolerance for median velocity validation at every iteration.
    mad_tol : float or None or tuple, optional
        Tolerance for median-absolute-deviation (MAD) velocity validation at every iteration.
    mean_tol : float or None or tuple, optional
        Tolerance for mean velocity validation at every iteration.
    rms_tol : float or None or tuple, optional
        Tolerance for root-mean-square (RMS) validation at every iteration.
    ...
    replacing_size : int or tuple, optional
        Size parameter for replacement kernel, kernel_size = 2 * size + 1.
    ...
    dtype_f : str, optional
        Float data type. Default of single is used if not specified.
    ...
    def __init__(self,
        ...
        self.num_passes = len(self.ss_iters)
        self.frame_shape = frame_shape
        self.overlap_ratio = (overlap_ratio,) * self.num_passes if overlap_ratio == 0 or \
            isinstance(overlap_ratio, float) else overlap_ratio
        self.shrink_ratio = shrink_ratio
        ...
        self.deforming_order = (deforming_order,) * self.num_passes if isinstance(deforming_order, int) else deforming_order
        self.is_normalized = (normalize,) * self.num_passes if isinstance(normalize, bool) else normalize
        self.subpixel_method = (subpixel_method,) * self.num_passes if isinstance(subpixel_method, str) else subpixel_method
        self.n_fft = (n_fft,) * self.num_passes if isinstance(n_fft, int) else n_fft
        self.deforming_par = (deforming_par,) * self.num_passes if deforming_par == 0 or deforming_par == 1 or \
            isinstance(deforming_par, float) else deforming_par
        ...
        self.num_replacing_iters = (num_replacing_iters,) * self.num_passes \
            if isinstance(num_replacing_iters, int) else num_replacing_iters
        self.replacing_method = (replacing_method,) * self.num_passes if isinstance(replacing_method, str) else replacing_method
        self.replacing_size = (replacing_size,) * self.num_passes if replacing_size is None or \
            isinstance(replacing_size, int) else replacing_size
        self.is_revalidated = (revalidate,) * self.num_passes if isinstance(revalidate, bool) else revalidate
        
        # Smoothing settings.
        self.is_smoothed = (smooth,) * self.num_passes if isinstance(smooth, bool) else smooth
        self.smoothing_par = (smoothing_par,) * self.num_passes if smoothing_par is None or \
            isinstance(smoothing_par, int) or isinstance(smoothing_par, float) else smoothing_par
    ...
    def __call__(self, frame_a, frame_b):
        ...
        for k in range(self.max_iters):
            ...
            # Set the prdictor to values from previous iteration.
            up, vp = u, v
    ...
    def interpolate_displacement(self, up, vp):
        """Interpolates the displacement from previous iteration onto the new grid."""
    ...
    def smooth_fields(self, u, v):
        """Returns smoothed velocity fields."""
    ...
    def scale_fields(self, u, v):
        """Returns scaled velocity fields."""
...
class PIVFieldCPU:
    ...
    overlap_ratio : float, optional
        Ratio of overlap (between 0 and 1) between interrogation and search windows.
    mask : ndarray or None, optional
    ...
    coords : tuple
        A tuple of 2D arrays, (x, y) coordinates, where the velocity field has been computed.
    grid_coords : tuple
        A tuple of 1D arrays, (x, y), containing the grid coordinates of the PIV field.
    
    """
    def __init__(self, frame_shape, window_size, search_size=None, overlap_ratio=0.5, mask=None, dtype_f=DTYPE_f):
        self.frame_shape = frame_shape
        self.window_size = window_size
        self.search_size = search_size if search_size is not None else window_size
        self.window_shape = (self.search_size, self.search_size)
        ...
        self.x_grid = self.x[0, :].astype(self.dtype_f)
        self.y_grid = self.y[:, 0].astype(self.dtype_f)
        self.mask = mask
...
class CorrelationCPU:
    ...
    Can perform correlation by extended search area, where the first window is larger than the second window,
    allowing a for displacements larger than the nominal window size to be found.
    ...
    def __init__(self, frame_a, frame_b,
        ...
        self.dtype_f = dtype_f
        self.dtype_c = DTYPE_c if dtype_f == DTYPE_f else np.complex64
    ...
    def __call__(self, piv_field, shift=None, n_fft=N_FFT, dt=DEFORMING_PAR, batch_size=BATCH_SIZE):
        ...
        piv_field : PIVFieldCPU
            Geometric information for the PIV field.
        shift : ndarray or None, optional
        ...
        dt : float, optional
            ...
            A default value of 0.5 is recommended to reduce bias errors.
        batch_size : int or "full", optional
            Batch size for cross-correlation.
        ...
        # Initialize FFT shape.
        self.init_fft_shape()
        
        # Get boolean array to select elements not inside the mask.
        self._field_mask = ~self.piv_field.field_mask if self.piv_field.field_mask is not None else None
        self._mask = self._field_mask.reshape(-1) if self.piv_field.field_mask is not None else None
        ...
        # Create a batch according to the batch size.
        self.batch_size = self.n_wins if batch_size == "full" or batch_size > self.n_wins else batch_size
        self.batch_indices = np.split(self.k_wins, range(self.batch_size, self.n_wins, self.batch_size))
        ...
        # Create a slice and set elements outside of the interrogation window to zero.
        if self.is_shrinked:
            self.i_ws = slice(self.piv_field.pad_ht, -self.piv_field.pad_ht)
            self.j_ws = slice(self.piv_field.pad_wd, -self.piv_field.pad_wd)
            win_a = self.zero_pad(win_a)
        
        if self.is_normalized:
            # Change data type to float if normalizing the intensity values.
            win_a, win_b = win_a.astype(self.dtype_f), win_b.astype(self.dtype_f)
            
            # Normalize only the elements inside the interrogation and search windows.
            if self.is_shrinked:
                win_a[:, self.i_ws, self.j_ws], win_b = self.normalize_intensity(win_a[:, self.i_ws, self.j_ws], win_b)
            else:
        ...
        self.i_peak1, self.j_peak1 = self.find_first_peak(self.corr)
        
        # Get the subpixel approximation of the first peak location.
        self.i_sp, self.j_sp = self.find_subpixel_peak(self.corr, self.i_peak1, self.j_peak1)
        
        # Get the displacement.
        self.u, self.v = self.find_displacement(self.i_sp, self.j_sp)
    ...
    def stack_windows(self, frame_a, frame_b, shift=None, deforming_order=1, dt=0.5):
        ...
        # Stack search windows.
        win_a = get_stack(frame_a, array_shape=self.piv_field.frame_shape,
                          window_size=self.piv_field.search_size, spacing=self.piv_field.spacing)
        win_b = get_stack(frame_b, array_shape=self.piv_field.frame_shape,
                          window_size=self.piv_field.search_size, spacing=self.piv_field.spacing)
        
        # Apply selection only if mask is provided.
        if self._field_mask is not None:
            win_a, win_b = win_a[self._mask], win_b[self._mask]
        
        return win_a, win_b
    
    def deform_frame(self, frame_a, frame_b, shift=None, deforming_order=1, dt=0.5):
        """Performs frame deformation by interpolating the intensity fields onto a new grid using
        a predictor field.
        """
    ...
    def zero_pad(self, win_a):
        """Sets the padded region in the first stack of windows to zeros."""
        win_a_padded = np.zeros((self.n_wins, self.piv_field.search_size, self.piv_field.search_size), dtype=self.dtype_f)
        win_a_padded[:, self.i_ws, self.j_ws] = win_a[:, self.i_ws, self.j_ws]
        return win_a_padded
    
    def normalize_intensity(self, win_a, win_b):
        """Remove the mean from each window of a 3D stack."""
        win_a -= win_a.mean(axis=(1, 2), keepdims=True)
        win_b -= win_b.mean(axis=(1, 2), keepdims=True)
        return win_a, win_b
    ...
    def correlate_windows(self, win_a, win_b):
        ...
        # Do cross-correlation except for the last batch.
        for k in self.batch_indices[:-1]:
    ...
    def batch_correlate_windows(self, win_a, win_b, f2a, f2b, corr, fft_forward, fft_inverse):
        ...
        # Perform inverse FFT.
        fft_inverse(f2a.conj() * f2b, corr)
        
        return corr
    
    def find_first_peak(self, corr):
        """Returns the row and column of the first peak in the correlation map."""
        corr = corr.reshape(self.n_wins, -1)
        indices = corr.argmax(axis=1)
        i_peak1 = indices // self.fft_ht
        j_peak1 = indices % self.fft_wd
        return i_peak1, j_peak1
    
    def find_subpixel_peak(self, corr, i_peak1, j_peak1):
    ...
    def find_displacement(self, i_sp, j_sp):
    ...
    def get_s2n_ratio(self):
        ...
        if self.s2n_method == 'peak2peak':
            self.i_peak2, self.j_peak2 = self.find_second_peak(self.corr, self.s2n_size)
            ...
            corr_peak2 = self.find_correlation_energy(corr)
        
        else:
            corr_peak2 = self.find_correlation_energy(self.corr)
    ...
    def find_second_peak(self, corr, width):
        """Returns the row and column of the second peak in the correlation map."""
        ...
        i_peak2, j_peak2 = self.find_first_peak(corr)
        
        return i_peak2, j_peak2
    
    def find_correlation_energy(self, corr):
